...
**現時点で、告示・省令・通知の原文に基づく項目は 0 件です。**

当たれているのは厚生労働省の審議会資料と算定構造（`agency`）、および民間の解説記事（`secondary`）までです。原文の文言と細かい例外規定は、まだこの手元にありません。
...
`status.json` が、検査器の出力そのままです。この README の数字も、そこから来ています。

| | |
|---|---|
| 版 | `2024-kaitei.seed.2`（令和6年度 介護報酬改定） |
| 項目数 | **8** |
| 出典 `statute`（告示・通知の原文） | **0 件** |
...
1. **告示・省令・通知の原文（`statute`）が 0 件。** 介護保険最新情報 Vol.1225 / Vol.1263 / Vol.1285 / Vol.1345 と、指定居宅サービス等の人員・設備・運営基準の該当条を取りに行きます。
2. **医療保険（訪問看護療養費）側が 0 項目。** 医療と介護のどちらで請求するかの判定は、この版では支えられません。
3. **加算が 2 項目のみ。** 取りこぼしを探すには全く足りません。
