...
**現時点で、告示・省令・通知の原文（`statute`）に基づく出典は 1 件だけです。**

その1件（厚生省令第37号）で取得できたのは、**訪問介護の章の条文**です。訪問看護へは準用規定を通じて適用されるはずですが、**その準用の条番号を、まだ確認できていません。** 条文を引いた分だけ、間違えたときに強く見えるので、「訪問看護にも同じ条文が適用される」とはまだ書いていません。
...
| | |
|---|---|
| 版 | `2024-kaitei.seed.3`（令和6年度 介護報酬改定） |
| 項目数 | **8** |
| 出典 `statute`（告示・省令・通知の原文） | **1 件** |
