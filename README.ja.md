...
**[English (front page) → README.md](README.md)**
...
<!-- auto:caution:start -->
**いま、現行の告示・省令・通知の原文（`statute`）に基づく出典は 8 件です。**（`statute` は全 11 件。うち 5 件は改正前・改定前の版で、**現行の根拠には使えません。**版を知るためだけに残しています。）

残りは厚生労働省の資料（`agency` 8 件）と、民間の解説（`secondary` 7 件）です。

**未確認の要件が 54 件あります。** 我々が原文で確かめられていないものは、そう書いてあります。空欄にはしていません。
<!-- auto:caution:end -->
...
| | |
|---|---|
| 版 | `2024-kaitei.seed.22` |
| 項目数 | **33** |
| 出典 `statute`（告示・省令・通知の原文） | 11 件 |
| うち **現行版** | **8 件** |
| 出典 `agency`（厚生労働省の資料） | 8 件 |
| 出典 `secondary`（民間の解説） | 7 件 |
| 出典 合計 | 26 件（うち現行でないもの 5 件） |
| 未確認の要件 | **54 件** |
| 食い違い | 全 4 件 / うち未解決 **1 件** |
...
さらに、**1日1回、本番に出ている版と、公式レジストリに載っている名刺を、リポジトリと突き合わせています**（上の `live` バッジ）。押したのに配っていない・**載せたつもりで載っていない**、を見つけるためです。**どちらも実際にやりました。**

この門は「**接続が立たなかったとき**」は赤にしません（届かないことは、古いことの証拠にならないため）。ただし「**HTTP の番号が返ったとき**」は答えとして扱い、200 以外なら赤にします。最初の版はここを一括りにしていて、**403 を受け取りながら毎日緑を出していました。** そして**その判定そのものを、偽の本番17通りで毎回の CI が確かめています**（1日1回しか走らない門が壊れていることは、壊れてから最大1日、誰にも分からないので）。
...
tools/test_live_workflow.py  1日1回の門(live.yml)の判定を、偽の本番17通りで確かめる
```
